...
public class WeeklyPayroll extends AppCompatActivity {
    //Navigation
    ...
    double totalregHours, totalregMins;
    ...
    TextView tv_hours, tv_overTimeHours, tv_totalAmount, tv_totalEmployees,tv_payrollStatus, tv_lastPayrollAmt, tv_payrollDate;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tv_payrollDate = findViewById(R.id.tv_payrollDate);
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                payrollAdapter = new PayrollAdapter(WeeklyPayroll.this,employeeNamesList, employeeImageList,employeePaycheckList);
                recyclerView.setAdapter(payrollAdapter);
                recyclerView.setLayoutManager(new LinearLayoutManager(WeeklyPayroll.this));
        ...
        btn_process.setOnClickListener(v->{
            ...
            if (!strDate.equals(tv_payrollDate.getText().toString()))
            {
                Toast.makeText(getApplicationContext(),"Payroll Date: " + strDate + " is not valid! Please process payroll on valid date!", Toast.LENGTH_LONG).show();
                return;
            }
    ...
    private void getCurrentWeekHours(String uid, String payrate) throws ParseException {
       ...
       // Date refDate = dateFormat.parse("12-10-2021");
        ...
        tv_payrollDate.setText(payDate);
                            ...
                            if (task.isSuccessful()) {
                                ...
                                if (document.exists()) {
                                    ...
                                    if (empNewHours.containsKey(uid))
                                    {
                                        ...
                                        // adding hours and storing in Database
    ...
    public void getMapHours( ArrayList<String> payrateList, Map<String,String> empList)
    {
        ...
        {   int checkHours = 0;
            ...
            Log.d("totalHours: ",  " Hours Above "+totalHours);
            Log.d("totalMinutes: ",  "Minutes Above "+totalMinutes);
            ...
            totalregHours = totalregHours + totalHours;
            totalMinutes = (float)totalregMins + totalMinutes;
            // Setting hours display
            if (totalMinutes == 0)
            ...
            // Clearing list for next employee
            hoursList.clear();
            minutesList.clear();
            ...
            if (okToFile)
            {
                ...
                Log.d("getHours", "Calling setDisplayAmount ");
                setDisplayAmount(uid,payrate);
            }
        }
    }


    public void setDisplayHours()
    {
        float h = 0;
        for( Float f : totalEmpRegHours)
        {
            h = h+f;
        }
        float ot = 0;
        for( Float m : totalEmpOTHours)
        {
            ot = ot +m;
        }

        tv_hours.setText(String.valueOf(h));
        tv_overTimeHours.setText(String.valueOf(ot));
    ...
    public void setAdminWeeklyData()
    {
        // Getting date ex: 11-22-2021
        Date currentDate = new Date();
        Calendar cal =  Calendar.getInstance(TimeZone.getDefault());
        ...
        // Creating documents reference for user in Firabase database
        DocumentReference docReference = fStore.collection("Users").document(fAuth.getUid())
                .collection("Payroll").document(strYear).collection("Week").document(strWeek);
        //Storing user info in map
        Map<String, Object> payrollInfo = new HashMap<>();
        payrollInfo.put("Date", date);
        payrollInfo.put("Amount",strTotalAmount);
        payrollInfo.put("Week",strWeek);
        docReference.set(payrollInfo).addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
            public void onSuccess(@NonNull Void unused) {
  ...
  /* For generating data - testing purpose
   public  void feedingData()
    {
        String uid = "zGg9yButOCRljSvYAG6ZgVgrJHx2";
